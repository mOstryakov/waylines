...
class AudioCombiner:
    def __init__(self):
        if not PYDUB_AVAILABLE:
            raise Exception("pydub is required for audio combining. Install with: pip install pydub")
            ...
            for i, point in enumerate(points, 1):
                if point.audio_guide:
                    try:
                        ...
                        point_audio = self.load_audio_file(point.audio_guide.path)
                        
                        # Добавляем паузу перед точкой (кроме первой)
                        if i > 1:
                            combined += AudioSegment.silent(duration=self.pause_duration)
                        
                        # Добавляем аудио точки
                        combined += point_audio
                        
                    except Exception as e:
                        logger.error(f"Ошибка загрузки аудио для точки {point.id}: {e}")
                        continue
