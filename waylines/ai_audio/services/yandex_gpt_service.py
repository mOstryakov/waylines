...
class YandexGPTService:
    def __init__(self):
        ...
        if not self.folder_id:
            raise ImproperlyConfigured(
                ...
                "Add to settings.py: YANDEX_FOLDER_ID = 'your_yandex_folder_id'"
            )
    ...
    def generate_location_description(
        ...
        lang_config = {
            "ru": {
                ...
                "live_guide": "Будь естественным, говори от первого лица как гид. Используй живую речь.",
            ...
            "kk": {
                ...
                "structure": (
                    "2. Негізгі сипаттама: бұл қандай орын, оның ерекшеліктері\n"
                    "3. Қызықты деректер мен тарихы (егер белгілі болса)\n"
                    "4. Осында не көруге/не істеуге болады\n"
                    "5. Туристке кеңестер\n"
                    "6. Қорытынды"
                ),
                "user_intro": "Координаталары бойынша орның экскурсия мәтінін жасаңыз",
                "address_fallback": "көрсетілмеген",
                "live_guide": "Табиғи болыңыз, гид ретінде бірінші жақтан сөйлеңіз. Тартымды тіл қолданыңыз.",
                "length": "Ұзындығы: 250–400 сөз.",
                "style_prompts": {
                    "storytelling": "тікелей гидтен айтылған әңгіме",
                    "historical": "орның тарихына назар аударатын тарихи әңгіме",
            ...
            "uz": {
                ...
                "user_intro": "Koordinatalar bo'yicha joy haqida ekskursiya matnini yarating",
                "address_fallback": "ko'rsatilmagan",
                "live_guide": "Tabiiy bo'ling, gid sifatida birinchi shaxsda gapiring. Jo'shqin til ishlating.",
                "length": "Uzunligi: 250–400 so'z.",
                "style_prompts": {
                    "storytelling": "tirik gid sifatida hikoya qilish",
                    "historical": "joyning tarixiga e'tibor qaratuvchi tarixiy hikoya",
            ...
            "en": {
                ...
                "structure": (
                    "2. Main description: what this place is, its key features\n"
                    "3. Interesting facts and history (if known)\n"
                    "4. What to see or do here\n"
                    "5. Tips for visitors\n"
                    "6. Conclusion"
                ),
                "user_intro": "Create a tour guide text for the location with these coordinates",
                "address_fallback": "not specified",
                "live_guide": "Be natural, speak in first person as a guide. Use engaging language.",
                "length": "Length: 250–400 words.",
                "style_prompts": {
                    "storytelling": "a storytelling narration as if from a live guide",
                    "historical": "historical narrative focusing on the place’s history",
                    "touristic": "practical tourist-oriented description",
                    "poetic": "poetic and artistic description of the location",
            ...
            "de": {
                ...
                "structure": (
                    "2. Hauptbeschreibung: Was ist dieser Ort, seine Merkmale\n"
                    "3. Interessante Fakten und Geschichte (falls bekannt)\n"
                    "4. Was man hier sehen oder unternehmen kann\n"
                    "5. Tipps für Besucher\n"
                    "6. Fazit"
                ),
                "user_intro": "Erstellen Sie einen Reiseführer-Text für den Ort mit diesen Koordinaten",
                "address_fallback": "nicht angegeben",
                "live_guide": "Seien Sie natürlich, sprechen Sie in der ersten Person als Guide. Verwenden Sie ansprechende Sprache.",
                "length": "Länge: 250–400 Wörter.",
                "style_prompts": {
                    "storytelling": "erzählender Stil wie von einem Live-Guide",
                    "historical": "historische Erzählung mit Fokus auf die Geschichte des Ortes",
                    "touristic": "praktische touristische Beschreibung",
                    "poetic": "poetische und künstlerische Beschreibung",
                    "scientific": "wissenschaftliche und faktenbasierte Beschreibung"
                }
            },
            "he": {
                ...
                "live_guide": "דבוק בסגנון טבעי, דבר בגוף ראשון כמדריך. השתמש בשפה חיה.",
        ...
        style_desc = config["style_prompts"].get(style, config["style_prompts"]["storytelling"])

        system_prompt = (
            f"{config['system_intro']} Create an engaging tour guide text about this location in the style of {style_desc}.\n\n"
            f"{config['structure']}\n\n"
            f"{config['live_guide']}\n"
            f"{config['length']}"
        )
        ...
        user_prompt = (
            ...
            f"Latitude: {lat}\n"
            f"Longitude: {lng}\n"
            f"Address: {address if address else config['address_fallback']}\n\n"
            "If a specific address is known, use it for a more accurate description.\n"
            "If it's a famous landmark, describe its history and significance.\n"
            "If it's an ordinary location, describe its atmosphere and surroundings.\n\n"
            "Important: Make the text lively and engaging, as if a guide is giving a live tour right here!"
        )
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(f"Yandex GPT error ({response.status_code}): {response.text[:200]}")

            result = response.json()
            generated_text = result["result"]["alternatives"][0]["message"]["text"]
            processing_time = time.time() - start_time
            logger.info(f"Yandex GPT: generated in {processing_time:.2f}s, {len(generated_text)} chars, lang={language}")
            return generated_text

        except Exception as e:
            logger.error(f"Yandex GPT error: {e}")
            return self._generate_fallback_description(lat, lng, address, style, language)
    ...
    def _generate_fallback_description(
        ...
        location_text = {
            "ru": f"Это место находится по адресу: {address}" if address else "Это интересное место с живописными окрестностями.",
            "kk": f"Бұл орын: {address}" if address else "Бұл қызықты жер, айналасы көркем.",
            "uz": f"Bu joy manzili: {address}" if address else "Bu qiziqarli joy, atrofi go'zal.",
            "en": f"This location is at: {address}" if address else "This is an interesting location with scenic surroundings.",
            "de": f"Dieser Ort befindet sich unter: {address}" if address else "Dies ist ein interessanter Ort mit malerischer Umgebung.",
            "he": f"המיקום הזה הוא ב: {address}" if address else "זהו מקום מעניין עם סביבה ציורית."
        }.get(language, f"This location is at: {address}" if address else "Interesting scenic location.")
...
Каждый камень, каждый изгиб ландшафта хранит свою историю..."""
            ...
            "en": {
                "storytelling": f"""Welcome to this unique location! My name is Matvey, and I’ll be your guide today.

{location_text} {coord_text}

Imagine we’re standing right here...""",
                "historical": f"""The location at {coord_text} has its own history and significance.
