...
class TTSService:
    ...
    def generate_audio(
        ...
        **kwargs
        ...
        lang_info = self.lang_voice_map.get(language, self.lang_voice_map["ru"])
        lang = lang_info["lang"]
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(f"Yandex TTS error ({response.status_code}): {response.text[:200]}")

            audio_content = response.content
            processing_time = time.time() - start_time
            logger.info(f"Yandex TTS: generated in {processing_time:.2f}s, lang={lang}, voice={final_voice}")
