...
class TTSService:
    ...
    def generate_audio(self, text, voice_type='alloy', language='auto', provider=None):
        ...
        try:
            ...
            audio_content = self.providers[provider].generate(text, voice_type, language)
            ...
            for backup_provider in ['yandex', 'google']:
                if backup_provider != provider:
                    try:
                        start_time = time.time()
                        audio_content = self.providers[backup_provider].generate(text, voice_type, language)
                        processing_time = time.time() - start_time
                        return audio_content, processing_time
                    except Exception as fallback_error:
                        logger.error(f"Fallback {backup_provider} also failed: {fallback_error}")
