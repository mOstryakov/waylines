    ...
    if created and instance.description.strip():
        try:
            ...
            if voice_profile.auto_generate:
                ...
                logger.info(f"Автогенерация аудио запущена для точки {instance.id}")
                
        except Exception as e:
            logger.error(f"Ошибка автогенерации аудио для точки {instance.id}: {e}")
...
def check_route_audio_completion(sender, instance, created, **kwargs):
    if instance.status == 'completed' and not instance.is_route_audio:
        ...
        points_with_audio = route.points.filter(audio_guide__isnull=False).count()
