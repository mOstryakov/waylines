...
def generate_audio(request, point_id):
    ...
    if point.route.author != request.user and not point.route.shared_with.filter(id=request.user.id).exists():
        return JsonResponse({'error': 'Нет доступа к этой точке'}, status=403)
    ...
    try:
        ...
        if not text.strip():
            return JsonResponse({'error': 'Текст для генерации пуст'}, status=400)
...
def get_audio_status(request, generation_id):
    ...
    return JsonResponse({
        ...
        'completed_at': audio_gen.completed_at.isoformat() if audio_gen.completed_at else None
    })
...
def generate_route_audio(request, route_id):
    ...
    if not points_with_audio.exists():
        return JsonResponse({'error': 'Нет точек с аудио для создания маршрута'}, status=400)
...
def get_route_audio_status(request, route_id):
    route_audio = get_object_or_404(RouteAudioGuide, route_id=route_id, route__author=request.user)
    
    return JsonResponse({
        'status': route_audio.status,
        'audio_url': route_audio.audio_file.url if route_audio.audio_file else None,
...
def generate_all_points_audio(request, route_id):
    route = get_object_or_404(Route, id=route_id, author=request.user)
    points_without_audio = route.points.filter(audio_guide__isnull=True, description__isnull=False).exclude(description='')
    
    if not points_without_audio.exists():
        return JsonResponse({'message': 'Все точки уже имеют аудио или не имеют описания'})
...
def get_route_points_audio_status(request, route_id):
    ...
    # Проверяем права доступа
    if route.author != request.user and not route.shared_with.filter(id=request.user.id).exists():
        return JsonResponse({'error': 'Нет доступа к этому маршруту'}, status=403)
    ...
    for point in points:
        ...
        latest_generation = audio_generations.first() if audio_generations.exists() else None
        
        audio_status.append({
            'point_id': point.id,
            'point_name': point.name,
            'has_audio': point.audio_guide is not None,
            'audio_url': point.audio_guide.url if point.audio_guide else None,
            'latest_generation_status': latest_generation.status if latest_generation else 'not_generated',
            'latest_generation_error': latest_generation.error_message if latest_generation else None
        })
    ...
    return JsonResponse({
        ...
        'points_with_audio': len([p for p in audio_status if p['has_audio']])
    })