...
def generate_point_audio(point_id, voice_type='alloy', language='auto'):
    ...
    try:
        ...
        if not text:
            logger.warning(f"Точка {point_id} не имеет описания для генерации аудио")
            return
        ...
        AudioGeneration.objects.filter(point_id=point_id, status='processing').update(
...
def generate_route_audio_guide(route_id):
    ...
    try:
        ...
        points = route.points.filter(audio_guide__isnull=False).order_by('order')
        ...
        route_audio, created = RouteAudioGuide.objects.get_or_create(route=route)
