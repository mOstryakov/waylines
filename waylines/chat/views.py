...
class ChatService:
    ...
    def get_or_create_conversation(user1, user2):
        ...
        if not conversation:
            with transaction.atomic():
                ...
                logger.info(f"Создан новый диалог между {user1.username} и {user2.username}")
    ...
    def validate_message_content(content):
        ...
        if len(content) > 1000:
            raise ValidationError("Сообщение слишком длинное (максимум 1000 символов)")
    ...
    try:
        ...
        if not public_chats:
            public_chats = RouteChat.objects.filter(
                route__privacy="public", 
                route__is_active=True
            ).select_related('route', 'route__author').exclude(
                route__author=request.user
            ).annotate(
                messages_count=Count('messages')
            ).order_by('-route__created_at')[:10]
            cache.set(public_chats_cache_key, public_chats, 300)  # Кеш на 5 минут
        ...
        ).distinct().only('id', 'username')[:20]  # Ограничиваем количество
        ...
        logger.error(f"Error in chat_dashboard for user {request.user.id}: {e}")
...
def route_chat(request, route_id):
    ...
    try:
        ...
        # Получаем сообщения с оптимизацией
        chat_messages = route_chat.messages.select_related('user').order_by("-timestamp")[:100]
...
def send_private_message(request):
    ...
    try:
        ...
        validated_content = ChatService.validate_message_content(message_content)

        other_user = get_object_or_404(User, id=user_id)
        
        # Проверяем, не пытается ли пользователь отправить сообщение самому себе
        if other_user == request.user:
            return JSONResponseMixin.error_response("Нельзя отправить сообщение самому себе")
        ...
        logger.error(f"Error sending private message for user {request.user.id}: {e}")
...
def send_route_message(request):
    ...
    try:
        ...
        logger.info(f"User {request.user.id} sent message to route chat {route_id}")
        ...
        logger.error(f"Error sending route message for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response(
...
def get_private_messages(request, conversation_id):
    ...
    try:
        ...
        if request.user not in conversation.participants.all():
            return JSONResponseMixin.error_response("Доступ запрещен", status=403)
        ...
        logger.error(f"Error getting private messages for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response("Ошибка загрузки сообщений", status=500)
...
def get_route_messages(request, route_id):
    ...
    try:
        ...
        # Базовый запрос с оптимизацией
        messages_qs = route_chat.messages.select_related('user').order_by("-timestamp")
        ...
        logger.error(f"Error getting route messages for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response("Ошибка загрузки сообщений", status=500)
...
def get_conversation_info(request, conversation_id):
    ...
    try:
        ...
        if not other_user:
            return JSONResponseMixin.error_response("Диалог не найден", status=404)
        ...
        logger.error(f"Error getting conversation info for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response("Ошибка загрузки информации", status=500)
...
def mark_conversation_as_read(request, conversation_id):
    ...
    try:
        ...
        logger.info(f"User {request.user.id} marked conversation {conversation_id} as read")
        ...
        logger.error(f"Error marking conversation as read for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response("Ошибка обновления статуса сообщений", status=500)
...
def get_unread_counts(request):
    ...
    try:
        ...
        conversations_unread = Conversation.objects.filter(
        ...
        return JSONResponseMixin.error_response("Ошибка загрузки счетчиков", status=500)
...
def delete_conversation(request, conversation_id):
    ...
    try:
        ...
        logger.info(f"User {request.user.id} deleted conversation {conversation_id}")
        ...
        logger.error(f"Error deleting conversation for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response("Ошибка удаления диалога", status=500)