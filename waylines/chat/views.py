...
class ChatService:
    ...
    def get_or_create_conversation(user1, user2):
        if user1 == user2:
            raise ValidationError(_("Cannot start a conversation with yourself"))
        conversation = (
    ...
    def validate_message_content(content):
        ...
        if len(content) > 1000:
            raise ValidationError(_("Message is too long (max 1000 characters)"))
        return content
    ...
    def get_route_chat_with_access_check(user, route_id):
        ...
        route_chat, _ = RouteChat.objects.get_or_create(route=route)
...
def chat_dashboard(request):
    ...
    try:
        ...
        existing_ids = {data["other_user"].id for data in conversations_data} if conversations_data else set()
...
def private_chat(request, user_id):
    ...
    try:
        accepted_friends_count = other_user.friendship_requests_sent.filter(status="accepted").count()
...
def route_chat(request, route_id):
    try:
        ...
        return render(request, "chat/route_chat.html", {
            "route": route, 
...
def send_private_message(request):
    try:
        ...
        if other_user == request.user:
            return JSONResponseMixin.error_response(_("Cannot send message to yourself"))
        ...
        return JSONResponseMixin.error_response(_("Internal server error"), status=500, error_code="server_error")
...
def send_route_message(request):
    try:
        ...
        return JSONResponseMixin.error_response(_("Failed to send message"), status=500)
...
def get_conversation_info(request, conversation_id):
    ...
    other_user = conversation.get_other_participant(request.user)
    if not other_user:
        return JSONResponseMixin.error_response(_("Conversation not found"), status=404)
...
def get_unread_counts(request):
    try:
        ...
        shared_routes_unread = (
            RouteChatMessage.objects.filter(
                route_chat__route__shared_with=request.user,
                is_read=False
            )
            .exclude(user=request.user)
            .count()
        ...
        logger.error(f"Error in get_unread_counts for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response(_("Failed to get unread counts"), status=500)
...
def delete_conversation(request, conversation_id):
    ...
    return JSONResponseMixin.success_response({"message": _("Conversation deleted")})
...
def mark_route_messages_as_read(request, route_id):
    try:
        ...
        logger.info(f"User {request.user.id} marked route messages for route {route_id} as read")
        return JSONResponseMixin.success_response({
            "success": True,
        ...
        logger.error(f"Error marking route messages as read for user {request.user.id}: {e}")
        return JSONResponseMixin.error_response(_("Failed to mark messages as read"), status=500)