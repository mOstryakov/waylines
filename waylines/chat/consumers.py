...
class PrivateChatConsumer(AsyncWebsocketConsumer):
    ...
    async def send_error(self, error_message):
        await self.send(
            text_data=json.dumps({"type": "error", "error": error_message})
        )
...
class RouteChatConsumer(AsyncWebsocketConsumer):
    ...
    def can_access_route(self):
        try:
            ...
            if route.privacy == "public":
                ...
                return (
                    route.author == self.user
                    or self.user in route.shared_with.all()
                )
            else:  # private
    ...
    @database_sync_to_async
    def save_message(self, message_content):
        route_chat, created = RouteChat.objects.get_or_create(
            route_id=self.route_id
        )
        message = RouteChatMessage.objects.create(
            route_chat=route_chat, user=self.user, message=message_content
        )
