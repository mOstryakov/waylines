...
class RouteModelsTest(TestCase):
    ...
    def test_route_favorite_creation(self):
        ...
        favorite = RouteFavorite.objects.create(
            route=route,
            user=self.user
        )
...
class RouteAPITest(TestCase):
    ...
    def test_toggle_favorite_api(self):
        ...
        self.assertTrue(RouteFavorite.objects.filter(
            user=self.user,
            route=self.route
        ).exists())
    ...
    def test_rate_route_api(self):
        ...
        self.assertTrue(RouteRating.objects.filter(
            user=self.user,
            route=self.route,
            rating=4
        ).exists())
...
class RoutePrivacyTest(TestCase):
    ...
    def test_public_route_access(self):
        ...
        response = client.get(reverse('route_detail', args=[self.public_route.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_private_route_access_owner(self):
        ...
        response = client.get(reverse('route_detail', args=[self.private_route.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_link_route_access(self):
        ...
        response = client.get(reverse('route_detail', args=[self.link_route.id]))
        self.assertEqual(response.status_code, 302)
...
class RouteCommentsTest(TestCase):
    ...
    def test_add_route_comment(self):
        ...
        self.assertTrue(RouteComment.objects.filter(
            route=self.route,
            user=self.user,
            text='Great route!'
        ).exists())
